...
export default function ConvertForm() {
  ...
  const handleConversion = () => {
    dispatch(setResult(convert(value).from(currentUnit).to(newUnit)));
  };
      ...
      <section className="form-group">
        <form>
          ...
          <label>
            ...
            <select name="newUnit" value={newUnit} onChange={handleInputChange}>
              {convert()
                .possibilities(category)
                .map((unit, index) => (
                  <option key={index} value={unit}>
        ...
        <button className="btn" type="submit" onClick={handleConversion}>
          Convert
        </button>
        <h2>
          {value} {currentUnit} equals {result} {newUnit}
        </h2>
      </section>
    </div>
  );
